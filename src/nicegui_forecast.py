...
def _get_start_prices(tickers: list, price_data: dict, currency: str) -> dict:
    ...
    for t in tickers:
        ...
        if close.empty:
            continue
        fx = get_fx_rate(get_ticker_currency(t), currency)
...
def _render_portfolio_outlook(
    ...
    # Horizon selector
    horizon_options = {"3 months": 62, "6 months": 125, "1 year": 251}
    ...
    caption_container = ui.column().classes("w-full")
    ...
    def _update_portfolio_outlook() -> None:
        ...
        div_label = "narrows" if div_diff < 0 else "widens"
            ...
            with ui.row().classes("w-full").style("gap:12px;"):
                with ui.column().style("flex:1; min-width:0;"):
                    _metric_card(
                        f"VaR 95% ({hl})",
                        f"{vc['var'] * 100:.1f}%",
                        f"{currency_symbol}{vc['var_abs']:,.0f}",
                    )
                with ui.column().style("flex:1; min-width:0;"):
                    _metric_card(
                        f"CVaR 95% ({hl})",
                        f"{vc['cvar'] * 100:.1f}%",
                        f"{currency_symbol}{vc['cvar_abs']:,.0f}",
                    )
                with ui.column().style("flex:1; min-width:0;"):
                    p10_chg = (corr_p10 - start_val) / start_val * 100
                    _metric_card(
                        "p10 outcome",
                        f"{currency_symbol}{corr_p10:,.0f}",
                        f"{p10_chg:+.1f}%",
                    )
                with ui.column().style("flex:1; min-width:0;"):
                    _metric_card(
                        "Diversification effect",
                        f"{currency_symbol}{abs(div_diff):,.0f}",
                        f"Correlation {div_label} p10",
                    )
...
def _render_position_outlook(
    ...
    # Controls row
    with ui.row().classes("w-full").style("gap:14px; align-items:flex-end;"):
        ticker_names = {t: f"{t} \u2014 {fetch_company_name(t)}" for t in tickers}
    ...
    def _update_position_outlook() -> None:
        ...
        fx = get_fx_rate(get_ticker_currency(ticker), currency)
...
def _render_backtest(
    ...
    <table>
        <thead><tr>
            <th>Ticker</th>
            <th>Hit Rate 80% CI</th>
            <th>Hit Rate 50% CI</th>
            <th>Kurtosis</th>
            <th>Skewness</th>
            <th>Fat-tailed</th>
            <th>Reliability</th>
...
def build_forecast_tab(portfolio: dict, currency: str) -> None:
    ...
    # Load simulation data once for both sections
    price_data = _load_simulation_data(tickers)
...
def build_diagnostics_tab(portfolio: dict, currency: str) -> None:
