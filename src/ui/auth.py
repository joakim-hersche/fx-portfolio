...
def _build_login_form(container, on_login_success: callable):
    ...
    with ui.column().classes("w-full items-center").style("padding-top:60px;"):
        with ui.card().style(
            ...
            # Guest promo code access
            promo_container = ui.column().classes("w-full").style("margin-top:12px;")
            ...
            def _toggle_promo():
                ...
                with promo_container:
                    ...
                    promo_error = ui.label("").style(
                        f"color:#EF4444; font-size:12px; min-height:18px; margin-top:2px;"
                    )

                    async def _use_code():
                        code = promo_code_input.value.strip() if promo_code_input else ""
                        if not code:
                            return
                    ...
                    ui.button("Access", on_click=_use_code).props("no-caps unelevated").style(
                        f"width:100%; margin-top:8px; background:{ACCENT}; border-radius:8px;"
                    )
